...
(async () => {
  ...
  const cssTwoSlash = fs.readFileSync(path.resolve(cwd(), 'styles/twoslash.css'), 'utf8');
  const cssDiff = fs.readFileSync(path.resolve(cwd(), 'styles/diff.css'), 'utf8');
  const cssShikiji = fs.readFileSync(path.resolve(cwd(), 'styles/shikiji.css'), 'utf8');
  ...
  fs.writeFileSync(path.resolve(cwd(), 'test/index.html'), templateHtml);
})();
